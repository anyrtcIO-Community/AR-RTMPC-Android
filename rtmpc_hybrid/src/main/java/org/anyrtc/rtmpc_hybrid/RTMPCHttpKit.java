...
public class RTMPCHttpKit {
    ...
    public static void forceCloseLive(final Context ctx, final String anyrtcid, final RTMPCHttpCallback callback) {
        ...
        final String httpurl = String.format(gHttpCloseActivity, RTMPCHybrid.Inst().getHttpAddr(), RTMPCHybrid.Inst().getStrAppId(),
                RTMPCHybrid.Inst().getStrAppKey(), anyrtcid);
                ...
                if (statusCode == 401 && auth_times == 0) {
                    for (int i = 0; i < headers.length; i++) {
                        if (headers[i].getName().equals("WWW-Authenticate")) {
                            ...
                            String response = authDigest("GET", RTMPCHybrid.Inst().getStrAppId(), realm, uri, RTMPCHybrid.Inst().getStrAppToken(), nonce, cnonce,
    ...
    public static void getLiveMemberList(final Context ctx, final String anyrtcId, String strRoomSevId, String strRoom, String strPageNum, final RTMPCHttpCallback callback) {
        ...
        final AsyncHttpClient httpClient = new AsyncHttpClient();
        final String httpUrl = String.format(gHttpLiveMemberListUrl, RTMPCHybrid.Inst().getHttpAddr(), RTMPCHybrid.Inst().getStrAppId(),
                RTMPCHybrid.Inst().getStrDeveloperId(), anyrtcId, strRoomSevId, strRoom, strPageNum);
        AsyncHttpResponseHandler handler = new AsyncHttpResponseHandler() {
            int auth_times = 0;
                ...
                if (statusCode == 401 && auth_times == 0) {
                    for (int i = 0; i < headers.length; i++) {
                        if (headers[i].getName().equals("WWW-Authenticate")) {
                            ...
                            String response = authDigest("GET", RTMPCHybrid.Inst().getStrAppId(), realm, uri,
                                    RTMPCHybrid.Inst().getStrAppToken(), nonce, cnonce,
                                    httpAuthHeader.getScheme(), httpAuthHeader.getQop().length() > 0);
    ...
    public static void getAuthLiveMemberList(final Context ctx, final String anyrtcId, String strRoomSevId, String strRoom, String strPageNum, final RTMPCHttpCallback callback) {
        ...
        final AsyncHttpClient httpClient = new AsyncHttpClient();
        final String httpUrl = String.format(gHttpLiveMemberListAuthUrl, RTMPCHybrid.Inst().getHttpAuthAddr(), RTMPCHybrid.Inst().getStrAppId(),
                RTMPCHybrid.Inst().getStrAppToken(), anyrtcId, strRoomSevId, strRoom, strPageNum);
        AsyncHttpResponseHandler handler = new AsyncHttpResponseHandler() {
            int auth_times = 0;
    ...
    public static void getLivingList(final Context ctx, final RTMPCHttpCallback callback) {
        ...
        final AsyncHttpClient httpClient = new AsyncHttpClient();
        final String httpUrl = String.format(gHttpLiveListUrl, RTMPCHybrid.Inst().getHttpAddr(), RTMPCHybrid.Inst().getStrAppId(),
                RTMPCHybrid.Inst().getStrDeveloperId());
        AsyncHttpResponseHandler handler = new AsyncHttpResponseHandler() {
            int auth_times = 0;
    ...
    public static void getAuthLivingList(final Context ctx, final RTMPCHttpCallback callback) {
        ...
        final AsyncHttpClient httpClient = new AsyncHttpClient();
        final String httpUrl = String.format(gHttpLiveListAuthUrl, RTMPCHybrid.Inst().getHttpAuthAddr(), RTMPCHybrid.Inst().getStrAppId(),
                RTMPCHybrid.Inst().getStrAppToken());
        AsyncHttpResponseHandler handler = new AsyncHttpResponseHandler() {
            int auth_times = 0;
    ...
    private static void recordRtmpStream(final Context ctx, final String strAnyrtcId, final String rtmpurl, final String resId,
        ...
        final AsyncHttpClient httpClient = new AsyncHttpClient();
        final String httpUrl = String.format(gHttpRecordUrl, RTMPCHybrid.Inst().getHttpAddr(), RTMPCHybrid.Inst().getStrAppId(),
                RTMPCHybrid.Inst().getStrDeveloperId(), strAnyrtcId, rtmpurl, resId);
        AsyncHttpResponseHandler handler = new AsyncHttpResponseHandler() {
            int auth_times = 0;
    ...
    private static void closeRecRtmpStream(final Context ctx, final String strVodSvrId, final String strVodResTag) {
        final AsyncHttpClient httpClient = new AsyncHttpClient();
        final String httpUrl = String.format(gHttpCloseRecUrl, RTMPCHybrid.Inst().getHttpAddr(), RTMPCHybrid.Inst().getStrAppId(),
                RTMPCHybrid.Inst().getStrDeveloperId(), strVodSvrId, strVodResTag);
        AsyncHttpResponseHandler handler = new AsyncHttpResponseHandler() {
            int auth_times = 0;
